...
def bios_serial_get(handle, server_id=1, caller="bios_serial_get"):
    server_dn = imccoreutils.get_server_dn(handle, server_id)
    dn = server_dn + "/bios/bios-settings/Console-redirection"
    mo = handle.query_dn(dn)
    if mo is None:
        raise ImcOperationError(caller,
                                "bios serial '%s' doesn't exist." % dn)
    return mo


def bios_serial_config(handle, vp_console_redirection=None,
                       vp_terminal_type=None, vp_baud_rate=None,
                       vp_flow_control=None, vp_putty_key_pad=None,
                       vp_redirection_after_post=None,
                       vp_legacy_os_redirection=None, server_id=1, **kwargs):

    mo = bios_serial_get(handle, server_id=server_id,
                         caller="bios_serial_config")
    params = {
        "vp_console_redirection": vp_console_redirection,
        "vp_terminal_type": vp_terminal_type,
        "vp_baud_rate": str(vp_baud_rate),
        "vp_flow_control": vp_flow_control,
        "vp_putty_key_pad": vp_putty_key_pad,
        "vp_redirection_after_post": vp_redirection_after_post,
...
def bios_serial_exists(handle, server_id=1, **kwargs):
    try:
        mo = bios_serial_get(handle, server_id=server_id,
                             caller="bios_serial_exists")
