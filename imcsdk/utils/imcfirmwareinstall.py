...
def firmware_exists(handle, version, server_id=1, force=False):
    if force:
        return False, None
...
def firmware_update(handle, remote_share, share_type, remote_ip,
                    ...
                    server_id=1, force=False, interval=60):

    version = version_extract(remote_share)
    if not force and firmware_exists(handle, version, force)[0]:
        return
...
def _has_upgrade_successful(update):
    if re.search(r"ERROR", update.overall_status):
        return False
    return True
